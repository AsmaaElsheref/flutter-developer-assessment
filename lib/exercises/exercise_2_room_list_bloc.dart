...
class FetchRoomsUseCase extends UseCaseWithParams<BaseResponse<List<RoomEntity>>, RoomParams> {
  @override
...
class RoomListBloc extends Bloc<RoomListEvent, RoomListState> {
  ...
  Future<void> _onLoadMore(
    ...
    // NOTE: Pagination should use a separate loading flag instead of updating the main status.
    // This prevents the entire list from disappearing during load more,
    // and allows showing a loading indicator at the bottom while keeping existing items visible.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<RoomListBloc, RoomListState>(
        ...
        buildWhen: (previous, current) => previous.status != current.status || previous.rooms.length != current.rooms.length,
        builder: (context, state) {
          switch (state.status) {
            case RequestState.idle:
            case RequestState.loading: return const Center(child: CircularProgressIndicator());
...
// ---------------------------------------------------------------------------
// SENIOR EXERCISE ANSWER
// ---------------------------------------------------------------------------
//
// 1. What's wrong with this BLoC?
// This BLoC is a "God Bloc" anti-pattern. It handles too many unrelated
// responsibilities such as user data, chat subscriptions, banners, games,
// analytics, wallet, app configuration, FCM token updates, and country data.
// This violates the Single Responsibility Principle and makes the BLoC too large
// and tightly coupled.
//
// 2. What problems does this cause?
// - Difficult to test because many dependencies must be mocked
// - Hard to maintain
// - High coupling between unrelated features
// - Increased risk of regressions when changing one concern
// - More complex lifecycle and side-effect management
//
// 3. How would you refactor it?
// I would split this BLoC into smaller feature-specific blocs/cubits.
// For example:
// - ProfileBloc for profile/user data
// - ChatBloc for chat subscriptions/messages
// - RealtimeBloc or PusherCubit for socket initialization
// - BannerBloc for banner events
// - GameBloc for game-related events
// - WalletBloc for wallet balance
// - ConfigCubit for app configuration
// - CountryCubit for countries/filters
// - NotificationCubit for FCM token updates